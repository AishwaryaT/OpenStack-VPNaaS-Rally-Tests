...
class VpnBase(rally_base.OpenStackScenario):
    ...
    def setup(self, **kwargs):
        ...
        with LOCK:
            ...
            self.remote_key_files = map(
                lambda x: "rally_keypair_" + x, self.suffixes)
            self.local_key_files = map(
                lambda x: '/tmp/' + x, self.remote_key_files)
            self.keypairs = []
            self.tenant_ids = []
        ...
        for x in range(MAX_RESOURCES):
            if self.tenant_ids:
                router, network, subnet, cidr = vpn_utils.create_network(
                    ...
                    DVR_flag=kwargs["DVR_flag"], ext_net=kwargs["ext-net"])
            else:
                router, network, subnet, cidr = vpn_utils.create_network(
                    self.neutron_client, self.neutron_admin_client,
                    self.suffixes[x], DVR_flag=kwargs["DVR_flag"],
                    ext_net=kwargs["ext-net"])
            with LOCK:
