...
def execute_cmd_over_ssh(host, cmd):
    ...
    """
    LOG.debug('EXECUTE COMMAND OVER SSH %s', cmd)
    ...
    LOG.debug("CONNECTED TO %s %s %s", host["ip"])
    try:
...
def write_key_to_compute_node(keypair, local_path, remote_path, host):
    ...
    :param keypair: nova keypair
    :param local_path: path to private key file in the local machine
...
def add_floating_ip(nova_client, server):
        ...
        if floating_ip is None:
            ...
            floating_ip = fp.ip
        LOG.debug("ASSOCIATING FLOATING IP %s", floating_ip)
...
def wait_for_namespace_creation(namespace, router_id, hosts, timeout=60):
    ...
    time exceeded.
    :param namespace: which namespace ("snat_" or "qrouter_")
...
def ssh_and_ping_server_with_fip(local_server, peer_server):
    ...
    :return: True if ping succeeds
             False if ping fails
    """
    LOG.debug("SSH INTO SERVER %s AND PING THE PEER SERVER %s",
              local_server, peer_server)
    count = 20
